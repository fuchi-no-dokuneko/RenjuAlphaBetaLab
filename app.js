(() => {
  ...
  function undo() { if (state.thinking) stopWorkers(); if (state.history.length <= 1) return; const remove = state.history.length >= 3 ? 2 : 1; state.history.splice(-remove); const last = state.history[state.history.length - 1]; state.board = Int8Array.from(last.board); state.turn = last.turn; state.over = false; state.replayIndex = state.history.length - 1; updateTurn(); draw(); setStatus("Last round removed."); }
  ...
  $("board").addEventListener("click", canvasMove); $("newGame").addEventListener("click", newGame); $("undo").addEventListener("click", undo); $("cheat").addEventListener("change", draw); $("replayBack").addEventListener("click",()=>replay(-1)); $("replayForward").addEventListener("click",()=>replay(1)); $("live").addEventListener("click",()=>{state.replayIndex=state.history.length-1;replay(0)}); window.addEventListener("resize", draw);
  window.RenjuApp = { state, newGame, humanMove, draw }; newGame();
})();
